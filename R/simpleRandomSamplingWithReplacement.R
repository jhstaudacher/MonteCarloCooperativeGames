...
simpleRandomSamplingWithReplacement <- function(i, n, m, v) {
  ...
  while (j < sampling_size) {
    j <- j + 1
    if (using_bigz) random_nr <- urand.bigz(1, length(players_without_i)) else random_nr <- sample.int(2^length(players_without_i), 1)
    sample <- coalitionFromIndex(players_without_i, random_nr)
    banzhaf_value <- banzhaf_value + (game(c(sample, player_i_value)) - game(sample))
  }
